...
1. **TriePrefixNotFoundError** — поднимается, когда требуемый для перехода префикс отсутствует
   в дереве;

2. **EncodingError** — поднимается при сбое кодирования текста из-за неправильного ввода или
   ошибки обработки;

3. **DecodingError** — поднимается при сбое декодирования текста из-за неправильного ввода или
   ошибки обработки;

4. **IncorrectNgramError** — поднимается при попытке использовать неподходящий размер n-грамм;

5. **MergeTreesError** — поднимается в случае, когда невозможно совершить слияние деревьев.
...
**Важно**: функционально эти токены очень похожи — они оба являются специальными символами,
...
2. Каждое предложение обрабатывается следующим образом:
   ...
   5. После последнего слова предложения добавляется `end_of_sentence_token`
...
`("hello", "world", "<EOS>", "how", "are", "you", "<EOS>")`.

Обратите внимание: для разделения предложений в данной работе используется именно
`end_of_sentence_token`, который в предыдущей лабораторной отвечал за разделение слов. Это сделано
...
`beautiful`. Изначально хранилище содержит в себе только `end_of_sentence_token`. По мере
вставки слов хранилище будет изменяться следующим образом:

1. `_storage = {'<EOS>': 0, 'hello': 1}` после вставки первого слова
2. `_storage = {'<EOS>': 0, 'hello': 1, 'world': 2}` после вставки второго слова
3. `_storage = {'<EOS>': 0, 'hello': 1, 'world': 2}` - ничего не происходит, слово `hello` уже есть
4. `_storage = {'<EOS>': 0, 'hello': 1, 'world': 2, 'beautiful': 3}` - после вставки последнего
   слова
...
2. Все непустые предложения обрабатываются следующим образом:
   ...
   3. В конец предложения должен быть добавлен ``end_of_sentence_token``, после чего все текущее
      предложение должно быть добавлено в список всех закодированных предложений

Обратите внимание, что метод должен возвращать объект типа `tuple` - массив всех закодированных
предложений.
...
обработки методом `encode_sentences` результат будет выглядеть так: `((1, 2, 0), (3, 4, 5, 0))`.
...
Например, метод получает на вход кортеж `("hello", "world", "<EOS>", "how", "are", "you", "<EOS>")`.
...
для поиска токенов будут доступны все вариации модели с разными n-граммами.
...
* ``_max_ngrams_size`` — наибольшее вероятное значение n-грамм для рассмотрения.
...
.. note:: В результате работы этого метода аргумент ``parent`` может измениться,
...
Для каждого дочернего узла копируемого дерева нужно найти место в
динамическом дереве, куда будет интегрироваться нужное поддерево.

Каждая итерация должна завершаться добавлением новой пары в стек:
текущего дочернего узла и узла, найденного среди дочерних узлов узла-приёмника.
...
.. note:: Если атрибут ``_models`` на момент вызова метода является пустым,
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Для успешного выбора и генерации следующего токена важно знать, на какой из размеров
...
От Вас требуется найти в дереве (начиная с дочерних узлов корня динамического дерева)
узлы, соответствующие значениям каждого из токенов контекста.
...
Если ни один дочерний узел не найден на любом из этапов, поиск прекращается,
и возвращается пустой словарь.

Для последнего найденного узла контекста следует рассмотреть каждого из его
...
.. important:: Выполнение Шагов 1-13 соответствует 10 баллам.
