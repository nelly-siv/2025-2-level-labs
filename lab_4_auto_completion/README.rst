...
5. **IncorrectCorpusError** — поднимается в случае, если используется неправильный корпус;

6. **MergeTreesError** — поднимается в случае, когда невозможно совершить слияние деревьев.
...
.. note:: Если корпус, который передаётся для обработки моделям, является пустым или
          некорректного типа, поднимите исключение **``IncorrectCorpusError``**.
